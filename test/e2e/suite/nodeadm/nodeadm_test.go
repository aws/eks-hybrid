	...
	When("using peered VPC", func() {
		...
		When("using ec2 instance as hybrid nodes", func() {
			...
			for _, osProvider := range suite.OSProviderList(credentialProviders) {
				...
				if !osystem.IsBottlerocket(os.Name()) {
			...
			DescribeTable("Joining a node",
				func(ctx context.Context, nodeOS e2e.NodeadmOS, provider e2e.NodeadmCredentialsProvider) {
					...
					verifyPodIdentityAddon := test.NewVerifyPodIdentityAddon(testNode.NodeInstance().Name, nodeOS.PodIdentityAgentDaemonsetName())
			...
			DescribeTable("Upgrade nodeadm flow",
				func(ctx context.Context, nodeOS e2e.NodeadmOS, provider e2e.NodeadmCredentialsProvider) {
					...
					Expect(test.NewUpgradeNode(testNode.NodeInstance().Name, testNode.NodeInstance().IP, testNode.OS).Run(ctx)).To(Succeed(), "node should have upgraded successfully")
