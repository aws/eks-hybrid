...
var _ = SynchronizedBeforeSuite(
	func(ctx context.Context) []byte {
		...
		// Shuffle lists for proper randomness
		...
		standardLinuxCpuIndex, standardLinuxGpuIndex := getRandomIndices(len(osList))
		bottlerocketCpuIndex, bottlerocketGpuIndex := getRandomIndices(len(bottlerocketList))

		// Create Standard Linux CPU node
		for i := range numberOfNodes {
			os := osList[standardLinuxCpuIndex].OS
			provider := osList[standardLinuxCpuIndex].Provider
		...
		for i := range numberOfNodes {
			os := bottlerocketList[bottlerocketCpuIndex].OS
			provider := bottlerocketList[bottlerocketCpuIndex].Provider
		...
		for i := range numberOfNodes {
			os := osList[standardLinuxGpuIndex].OS
			provider := osList[standardLinuxGpuIndex].Provider
		...
		for i := range numberOfNodes {
			os := bottlerocketList[bottlerocketGpuIndex].OS
			provider := bottlerocketList[bottlerocketGpuIndex].Provider
...
func getRandomIndices(length int) (int, int) {
	if length < 2 {
		return 0, 0
	}
