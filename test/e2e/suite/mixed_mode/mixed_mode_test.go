	...
	When("hybrid and cloud-managed nodes coexist", func() {
		...
		Context("CloudWatch Observability Mixed Mode", func() {
			It("should support CloudWatch Observability webhook functionality in mixed mode", func(ctx context.Context) {
				...
				err := cloudwatchAddon.SetupCwAddon(ctx, test.EKSClient, test.IAMClient, test.K8sClient.Interface, test.AWS, test.Logger)
				...
				err = cloudwatchAddon.VerifyCwAddon(ctx, test.K8sClient.Interface, test.AWS, test.Logger)
