...
func (r *retryable) recover() {
	...
	if !ok {
		r.panicableError = types.GinkgoError{
			Heading: "FlakyCode failure",
			Message: fmt.Errorf("unknown panic: %v", e).Error(),
		}
		return
	}
...
func (f *FlakyCode) It(ctx context.Context, description string, flakeAttempts int, body func(ctx context.Context, flakeRun FlakeRun)) {
	for attempt := range flakeAttempts {
		...
		// so when we expect here, the stacktrace and error message are pulled from what was stored via Failed
		// so we do not need to control the stacktrace output in this case
