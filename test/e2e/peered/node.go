...
func (c NodeCreate) Create(ctx context.Context, spec *NodeSpec) (PeeredNode, error) {
	...
	if err := ec2.WaitForEC2InstanceRunning(ctx, c.EC2, instance.ID); err != nil {
		return PeeredNode{}, fmt.Errorf("waiting for EC2 instance for node to be running: %w", err)
	}
...
func (c *NodeCleanup) Cleanup(ctx context.Context, node PeeredNode) error {
	...
	defer cancel()
	err := c.collectLogs(logCtx, constants.LogCollectorBundleFileName, c.OS, node.Instance)
	if err != nil {
...
func (c *NodeCleanup) NodeInfrastructureCleaner(node PeeredNode) *NodeInfrastructureCleaner {
...
func (c NodeCleanup) collectLogs(ctx context.Context, bundleName, os string, instance ec2.Instance) error {
	...
	collector := nodeadm.NewNodeLogCollector(os)
	err = collector.CollectLogs(ctx, c.RemoteCommandRunner, instance.IP, url)
	if err != nil {
		return err
	}
	return nil
