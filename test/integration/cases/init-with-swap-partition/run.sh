...
exit_code=0
STDERR=$(nodeadm init --config-source file://config.yaml --skip node-ip-validation,k8s-endpoint-network-validation,k8s-authentication-validation 2>&1) || exit_code=$?
