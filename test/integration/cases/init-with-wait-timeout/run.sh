...
# Test init with custom wait timeout (30 seconds)
echo "Testing nodeadm init with custom wait-timeout of 30s"
nodeadm init --skip run,preprocess,node-ip-validation,k8s-authentication-validation --config-source file://config.yaml --wait-timeout 30s

assert::path-exists /root/.aws
assert::path-exists /eks-hybrid/.aws

# Test init with disabled wait timeout (0s)
echo "Testing nodeadm init with disabled wait-timeout (0s)"
...
nodeadm install $CURRENT_VERSION --credential-provider ssm
mock::ssm
nodeadm init --skip run,preprocess,node-ip-validation,k8s-authentication-validation --config-source file://config.yaml --wait-timeout 0s
...
echo "All wait-timeout tests passed successfully"
