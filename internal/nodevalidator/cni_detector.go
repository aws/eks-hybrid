...
func (cd *cniDetector) DetectCNI(ctx context.Context, nodeName string) (CNIType, error) {
	...
	if err != nil && cniType == CNITypeNone {
		return cniType, fmt.Errorf("cni config file: %w", err)
	}

	cd.logger.Info("detected cni config file", zap.String("cniType", string(cniType)))

	cniType, err = cd.detectFromBinaries()
	if err != nil && cniType == CNITypeNone {
		return cniType, fmt.Errorf("cni binary files: %w", err)
	}

	cd.logger.Info("detected cni binary files", zap.String("cniType", string(cniType)))
	...
	if nodeName != "" {
		...
		// Taint-based detection, validate only if node runtime condition failed
