...
func (d *Delete) Run(log *zap.Logger, opts *cli.GlobalOptions) error {
	...
	if os.IsBottlerocket(osArch) {
		logCollector = peered.BottlerocketLogCollector{
			Runner: ssm.NewSSHOnSSMCommandRunner(ssmClient, *jumpbox.InstanceId, "ec2-user", logger),
		}
	} else {
		logCollector = peered.BottlerocketLogCollector{
			Runner: ssm.NewSSHOnSSMCommandRunner(ssmClient, *jumpbox.InstanceId, "root", logger),
		}
	}
